import time
import Read_Blogs_From_Rss
import Read_Blogs_From_CSV
import Read_Blogs_From_Inbox
import json
import os
import re
import nest_asyncio
...
load_dotenv()
blog_rss_items = Read_Blogs_From_Rss.Extract_RSS_Blog_Links()
blog_csv_items=Read_Blogs_From_CSV.Read_Blogs_CSV()
...
def read_gitcoin_blogs():
    return blog_csv_items.Gitcoin_Blog()

@time_function
def read_email_gitcoin_blogs():
    return blog_csv_items.Email_Gitcoin_Blogs()

@time_function
def read_huddle01_blogs():
    return blog_csv_items.Huddle01_Blog()


@time_function
def read_funding_commons_blogs():
    return blog_csv_items.FundingComoons_Blog()

@time_function
def read_email_funding_commons_blogs():
    return blog_csv_items.Email_Funding_The_Commons_Blogs()

@time_function
def read_libp2p_blogs():
    return blog_csv_items.Libp2p_Blog()


@time_function
def read_lighthouse_blogs():
    return blog_csv_items.LightHouse_Blog()


@time_function
def read_drand_blogs():
    return blog_csv_items.Drand_Blog()


@time_function
def read_tableland_blogs():
    return blog_csv_items.Tableland_Blog()


@time_function
def read_desci_blogs():
    return blog_csv_items.Desci_Blog()


@time_function
def read_oso_blogs():
    return blog_csv_items.OSO_Blog()


@time_function
def read_dsa_blogs():
    return blog_csv_items.DSA_Blog()


@time_function
def read_molecule_blogs():
    return blog_csv_items.Molecule_Blog()

@time_function
def read_email_molecule_blogs():
    return blog_csv_items.Email_Molecule_Blogs()

@time_function
def read_textile_blogs():
    return blog_csv_items.Textile_Blog()
...
async def read_urls_from_rss():
    ...
    return filecoin_docs, ipfs_docs,protocol_labs_docs


def read_email_extract_docs(username,password,team):
    imap_login = Read_Blogs_From_Inbox.Extract_Teams_Blogs(username, password)
    if team=="Gitcoin":
        gitcoin_blog_urls = imap_login.Email_Gitcoin_Blogs()
        gitcoin_docs = email_url(gitcoin_blog_urls)
        return gitcoin_docs
    if team == "Molecule":
        molecule_blog_urls = imap_login.Email_Molecule_Blogs()
        molecule_docs=email_url(molecule_blog_urls)
        return molecule_docs
    if team == "Funding The Commons":
        funding_common_blog_urls = imap_login.Email_Funding_The_Commons_Blogs()
        funding_common_docs = email_url( funding_common_blog_urls)
        return  funding_common_docs


async def read_urls_from_email():
    username = os.getenv("Mail_Box")
    password = os.getenv("Pass")
    all_docs=[]
    email_blog_teams=['Gitcoin','Molecule','Funding The Commons']
    for teams in email_blog_teams:
        docs=read_email_extract_docs(username,password,teams)
...
async def main():
    ...
    all_docs_rss = await read_urls_from_rss()
    all_docs_email= await read_urls_from_email()
    all_docs_csv= [doc for sublist in all_docs_csv for doc in sublist]
    all_docs_email=[doc for sublist in all_docs_email for doc in sublist]
    all_docs_rss=[doc for sublist in all_docs_rss for doc in sublist]
    print(len(all_docs_csv))
    cleaned_docs_csv = data_cleaning(all_docs_csv)
    cleaned_docs_email=data_cleaning(all_docs_email)
    qdrant_csv= await create_vector_qdrant(cleaned_docs_csv)
    qdrant_rss =await create_vector_qdrant(all_docs_rss)
    qdrant_email= await create_vector_qdrant(cleaned_docs_email)
