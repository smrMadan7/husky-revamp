...
class graph_retrieval:
    ...
    def collect_company(self, search_term, result):
        ...
        for record in result:
            company_data = {"name": "", "about": "", "directory_link": "", "contact": "", "funding_stage": "", "industry_tags": [], "focus_area": [], "technology": []}
            node = record[0]
    ...
    def collect_projects(self, search_term, result):
        ...
        print("super------------->",result)
        for record in result:
            projects_data = {"name": "", "directory_link": "", "contact_email": "", "tagline": ""}
            node = record[0]
            connections = record['connections']
    ...
    def collect_members(self, search_term, result):
        ...
        print(result)
        for record in result:
            members_data = {"name": "", "organization": "", "directory_link": "", "role": [], "office_hours": ""}
            node = record[0]
            ...
            for connection in incoming_connections:
                if connection['relationship'] == "HAS_MEMBER":
                    ...
                    organization = list(organization_node.items())[0][1] if organization_node else ""
                    if organization:
    ...
    #     final_company_data = []
    #     for record in result:
    #         company_data = {"name": "", "about": "", "directory_link": "", "contact": "", "funding_stage": "",
    ...
    #         connections = record['connections']
    #
    #         # Debug print to check node properties
    #
    #         # Extract directory link directly from node properties
    #         projects_data["name"] = node["name"]
    #         projects_data["directory_link"] = node.get("directoryLink", "")  # Use the correct key here
    #
    #         print("uuuuuuu-------------->", projects_data)
    #
    #         for connection in connections:
    ...
    #     return final_projects_data
        ...
        #                         projects_data["contact_email"] = [contact_email]
        #
        #             if relationship == "HAS_DIRECTORY_LINK" and connected_node:
        #                 directory_link = connected_node.get("url", "")
        #                 if directory_link:
        #                     if isinstance(directory_link, list):
        #                         projects_data["directory_link"] = directory_link
        #                     else:
        #                         projects_data["directory_link"] = [directory_link]
        #         final_projects_data.append(projects_data)
        #
    ...
    #         connections = record['outgoing_connections']
    #         incoming_connections = record['incoming_connections']
    #         print("hhhhh-------------->",incoming_connections)
    #         members_data["name"] = node["name"]
    #         members_data["organization"] = []
    #
    #         for connection in incoming_connections:
    #             print("Incoming connection:", connection)  # Debugging statement
    #             if connection['relationship'] == "HAS_MEMBER":
    #                 organization_node = connection.get("connected_node", {})
    #                 print("Organization node:", organization_node)  # Debugging statement
    #                 organization_name = organization_node.get("name", "") if organization_node else ""
    #                 print("Extracted organization name:", organization_name)  # Debugging statement
    #                 if organization_name:
    #                     members_data["organization"] = organization_name
    #
    #
    #         # Handling connections
    #         for connection in connections:
    ...
    def extract_entities(self,query, answer: str, references: List[Tuple[str, str]]) -> Dict[str, Any]:
        if references:
            extracted_sources = [item[1] for item in references]
            reference = "\n".join(extracted_sources)
        else:
            reference = ""
        text = " ".join([query,answer, reference])
...
#     retrieval_obj = graph_retrieval()
#     search_terms = ['Juan Benet','IPFS','Fluence','Gitcoin']
#     final_answer=retrieval_obj.get_answer(search_terms)
